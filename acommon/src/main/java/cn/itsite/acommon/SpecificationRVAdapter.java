...
public class SpecificationRVAdapter extends BaseRecyclerViewAdapter<String, BaseViewHolder> {
    private static final String TAG = SpecificationRVAdapter.class.getSimpleName();
    ...
    protected void convert(final BaseViewHolder helper, String item) {
        ...
        if (flexboxLayout.getChildCount() == 0) {
            for (int i = 0; i < Math.random() * 15; i++) {
                ...
                textView.setOnClickListener(v -> textView.setSelected(!textView.isSelected()));
                flexboxLayout.addView(view);
            }
            flexboxLayout.post(() -> notifyItemChanged(helper.getLayoutPosition()));
        }
    }
}
